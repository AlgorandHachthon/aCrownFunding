  ...
  return createApp(account, day, amount).then(async (re) => {
    ...
    return await getTxId(createTxnID).then(async (appid) => {
      updateApp(account, appid).then(async (update) => {
        ...
        var ApprovalProgram = Uint8Array.from(
          Buffer.from(appApprovalProgram, "base64")
        );
        var ClearProgram = Uint8Array.from(
          Buffer.from(appClearProgram, "base64")
        );
...
export var donateApp = async (account, app, amount) => {
  ...
  var to = algosdk.encodeAddress(
    Uint8Array.from(Buffer.from(app.Escrow, "base64"))
  );
  let params = await AlgoSigner.algod({
    ledger: "TestNet",
    path: "/v2/transactions/params",
  ...
  let binarySignedoptTx = AlgoSigner.encoding.base64ToMsgpack(
    signedoptTxs[0].blob
  );
  await algodclient.sendRawTransaction(binarySignedoptTx).do();
  var appArgs = [Uint8Array.from(Buffer.from("donate"))];
  let apptran = makeApplicationNoOpTxn(account, sugparams, app.id, appArgs);
  console.log(to);
  ...
  var signedTx1Binary = AlgoSignerSignedTxnToSDKRaw(signed[0]);
  var signedTx2Binary = AlgoSignerSignedTxnToSDKRaw(signed[1]);
  await algodclient.sendRawTransaction([signedTx1Binary, signedTx2Binary]).do();
...
export var claimApp = async (account, app) => {
  await algoDetect();
  var from = algosdk.encodeAddress(
    Uint8Array.from(Buffer.from(app.Escrow, "base64"))
  );
  let params = await AlgoSigner.algod({
    ledger: "TestNet",
